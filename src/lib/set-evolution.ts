...
// Sans série de référence : "Nouvelle". Rien n'a bougé : "=".
export function setEvolution(current: SetValues, previous: SetValues | undefined): EvolutionBadge[] {
  ...
  if (repsDiff !== 0) {
    ...
    badges.push({
      ...
      label: `${repsDiff > 0 ? "+" : "−"}${formatReps(reps)} rep${reps > 1 ? "s" : ""}`,
    });
  }
  return badges;
}
